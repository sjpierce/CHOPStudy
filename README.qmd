...
incidence of bandemia in canine patients undergoing chemotherapy treatment 
...
version 2025.05.1+513 (or later). We recommend using RStudio to interact with
...
reports [@Mair-RN3387]. Install Quarto version 1.7.33 or later. Although 
    ...
    * `scripts/` The folder is configured as a Quarto project. It holds 
      meta-data files, `.qmd` scripts, and files used by the scripts. 
        ...
        * `compact-title.tex`: This LaTeX file can be called from an .qmd file's
           ...
           the rendered PDF file so it uses less vertical white space. It may be 
        ...
        * `Descriptive_Analysis.qmd`: This file runs some descriptive analyses 
          we plant to use in our manuscript. 
...
the `scripts/output` folder. It will also generate `scripts/Render_Scripts.html`, 
...
the commands in the Terminal to render that script. The first line changes to 
the `scripts/` subfolder and the 2nd one does a custom render of that script,
setting matching custom values for the output file name and the LogFile 
parameter.
