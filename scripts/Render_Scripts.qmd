...
render this entire file to automate rendering all the scripts shown below to 
date-stamped, production HTML output files. 
...
:::{.callout-caution}
Rendering the import script in the `Import-Data` chunk below will overwrite the 
existing `data/Imported_CHOP_Data.RData` file in your local repository. If you 
just want to reproduce the analyses without re-running the import, disable the
`Import-Data` chunk by setting its chunk option to `#| eval: false` before you
render this production script.
:::
